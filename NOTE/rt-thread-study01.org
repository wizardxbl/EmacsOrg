...
#+END_SRC
这里 =RT_THREAD_PRIORITY_MAX= 的值为256，表示有256个优先级

这个优先级数组里面的内容都是链表的节点，只要知道了某个优先级，那么就可以直接指向对应的线程入口了。

优先级数值越小的优先级别越高，因此，这个问题可以变成，找出数组中第一个非空的元素
#+BEGIN_SRC c
for (i=0; i<256; i++) {
    if (rt_thread_priority_table[i] != NULL) 
        break;
}
highest_ready_priority = i; //最高优先级
#+END_SRC
...
现在的约定是，这32个byte就是256bit，和256个线程的优先级是一一对应的。

每个字节的最高位为bit7,最低位为bit0,用bit0表示更高的优先级，bit7表示稍低的优先级。
...
对于一个优先级是125的线程，在它的状态变为READY的时候，就会改变这个数组对应的字节的对应的位。

对应关系是125/8=15(字节), 125%8=5(位)

现在我们面临的问题是，寻找优先级最高的线程的问题，就变成了，从位图中找出第一个为1的bit的位置。

比如有A,B,C三个线程，优先级分别为A-5,B-25,C-125，那么位图中的BIT5,BIT25,BIT125就分别为1.
...
for(i=0; i<32; i++) {
    for(j=0; j<8; j++) {
	    ...
	    break;//找到最低位为1的bit的位置
    }
    highest_ready_priority = i*8+j;//最高优先级
}
#+END_SRC

依然，循环不是个节约时间的算法,那么就要想别的方法
